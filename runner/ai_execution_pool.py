...
def task(raw):
    ...
    return {'taskId':_id(raw.get('taskId'),'taskId'),'projectId':_id(raw.get('projectId'),'projectId'),'workstreamId':_id(raw.get('workstreamId'),'workstreamId'),'runId':_id(raw.get('runId'),'runId'),'taskType':tt,'state':st,'createdAt':_iso(raw.get('createdAt'),'createdAt'),'paid':paid,'explicitApproval':bool(raw.get('explicitApproval',False)),'automaticRetry':False,'estimatedCostUsdMicros':est,'candidateRef':None if raw.get('candidateRef') is None else _id(raw.get('candidateRef'),'candidateRef'),'authorityRef':None if raw.get('authorityRef') is None else _id(raw.get('authorityRef'),'authorityRef'),'blockedReason':None if raw.get('blockedReason') is None else str(raw.get('blockedReason'))[:500],'startedAt':None if raw.get('startedAt') is None else _iso(raw.get('startedAt'),'startedAt'),'completedAt':None if raw.get('completedAt') is None else _iso(raw.get('completedAt'),'completedAt')}
def receipt(raw):
...
def self_test():
    ...
    z='2026-08-27T00:00:00Z';s['tasks']=[_task('a-1','a','ws-a','run-a1','CODING_WORKER',z),_task('a-2','a','ws-a','run-a2','AI_REVIEW','2026-08-27T00:00:01Z'),_task('b-1','b','ws-b','run-b1','CODING_WORKER',z),_task('c-r1','c','ws-c','run-c1','AI_REVIEW',z,True,True,10,'sha:abc'),_task('c-r2','c','ws-c','run-c2','AI_REVIEW','2026-08-27T00:00:01Z',True,True,10,'sha:abc'),_task('u-1','u','ws-u','run-u1','AI_REVIEW',z,True,True,10)]
