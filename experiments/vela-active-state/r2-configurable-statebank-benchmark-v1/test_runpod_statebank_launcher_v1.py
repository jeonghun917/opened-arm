    ...
    def fake_sh(self, cmd, **kw):
        ...
        if cmd[:3] == ["runpodctl", "pod", "get"]:
            ...
            if self.mode == "restart":
                uptime = [30, 35, 4][min(self.get_count - 1, 2)]
            else:
                uptime = 20 + self.get_count * 3
            return SimpleNamespace(
                returncode=0,
                stdout=json.dumps(
                    {
...
def run_case(mode: str) -> tuple[object, Harness, int]:
    ...
    mod.image_ref = lambda evidence: mod.PINNED_IMAGE
...
def main() -> None:
    ...
    assert "--terminate-after" in success.create_command
    ...
    assert cancelled.delete_count == 1
