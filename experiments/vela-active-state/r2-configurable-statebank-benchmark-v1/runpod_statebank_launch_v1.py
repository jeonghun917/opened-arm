...
def image_ref(evidence):
 tag='python:3.12.13-slim-bookworm';raw=sh(['docker','buildx','imagetools','inspect',tag,'--raw'],capture_output=True,check=True).stdout.encode();(evidence/'image-manifest.json').write_bytes(raw);j=json.loads(raw)
 if 'manifests' in j:
  m=next(x for x in j['manifests'] if (x.get('platform') or {}).get('os')=='linux' and (x.get('platform') or {}).get('architecture')=='amd64');return 'python@'+m['digest']
 return 'python@sha256:'+hashlib.sha256(raw).hexdigest()
def main():
