...
def redact_text(text,token,sup):
 return text.replace(token,'<redacted-result-token>').replace(sup,'<redacted-supervisor-b64>')
def save_create_evidence(raw,path,token,sup):
 try:
  obj=json.loads(raw)
  if isinstance(obj.get('env'),list):
   obj['env']=[(('VELA_RESULT_TOKEN=<redacted>' if x.startswith('VELA_RESULT_TOKEN=') else 'VELA_SUPERVISOR_B64=<redacted>' if x.startswith('VELA_SUPERVISOR_B64=') else x)) for x in obj['env']]
  path.write_text(json.dumps(obj,indent=2,sort_keys=True)+'\n')
 except Exception:
  path.write_text(json.dumps({'unparsed_sha256':hashlib.sha256(raw.encode()).hexdigest(),'bytes':len(raw)})+'\n')
def capture_logs(pod,evidence,token,sup,label):
 for source in ('system','container'):
  r=sh(['runpodctl','pod','logs',pod,'--since','3m','--source',source],capture_output=True)
  body=redact_text((r.stdout or '')+(('\nSTDERR:\n'+r.stderr) if r.stderr else ''),token,sup)
  (evidence/f'{label}-{source}.log').write_text(body)
  tail=body[-1600:].replace('\n',' | ')
  emit('VELA_POD_LOG_SNAPSHOT',source=source,exit_code=r.returncode,tail=tail)
def main():
 ...
 (evidence/'pod-id.txt').write_text(pod+'\n');proxy=f'https://{pod}-{result_port}.proxy.runpod.net';start=time.time();found=False;terminal='';deleted=False;proxy_seen=False;last_report=0.0;last_phase=None;last_diag=0.0
 ...
 try:
  while time.time()-start<max_runtime:
   ...
   if code==200:
    ...
    if state=='FAILED':terminal='SUPERVISOR_FAILED';capture_logs(pod,evidence,token,sup,'failure');break
    if state=='COMPLETE':
     ...
     terminal=f'RESULT_FETCH_FAILED_{rc}';capture_logs(pod,evidence,token,sup,'result-fetch-failure');break
   else:
    ...
    if now-start>=30 and now-last_diag>=60:capture_logs(pod,evidence,token,sup,f'proxy-debug-{int(now-start)}s');last_diag=now
    if not proxy_seen and now-start>=120:terminal='PROXY_START_TIMEOUT';capture_logs(pod,evidence,token,sup,'proxy-start-timeout');break
   time.sleep(3)
  if not terminal:terminal='MAX_RUNTIME_EXPIRED';capture_logs(pod,evidence,token,sup,'max-runtime')
 finally:
  emit('VELA_POD_DELETE_START',pod_id=pod,terminal_reason=terminal);d=sh(['runpodctl','pod','delete',pod],capture_output=True);(evidence/'pod-delete.stdout').write_text(redact_text(d.stdout,token,sup));(evidence/'pod-delete.stderr').write_text(redact_text(d.stderr,token,sup));deleted=(d.returncode==0 or 'not_found' in d.stderr);emit('VELA_POD_DELETE_DONE',pod_id=pod,deleted=deleted,exit_code=d.returncode)
