...
def run_condition(model,pipe,problem,condition,cfg,retrieval):
 ...
 for ep in problem['episodes']:
  ...
  if inp=='zero': snap=engine.clone_snap(zero)
  elif inp=='selected':
   if bank not in banks: raise RuntimeError(f'missing bank {bank}')
   snap=engine.clone_snap(banks[bank])
  else: raise RuntimeError(f'unsupported input_state {inp}')
  before=engine.state_digest(snap)
  if mode.get('probe'):
   prompt=metrics.render_probe_prompt(ep,problem,retrieval_cfg); parsed,raw,u=engine.greedy_json_probe(model,pipe,snap,prompt,int(cfg['generation']['max_new_tokens'])); units+=u; probes.append(metrics.evaluate_probe(problem,ep,parsed,raw)); after=before
  else:
   _,out,u=engine.feed_text(model,pipe,snap,str(ep['model_input'])); units+=u; commit=mode['commit']
   if commit=='all':
...
def aggregate_results(cfg,results):
 ...
 for c in cfg.get('comparisons',[]):
  t,ctl=c['treatment'],c['control']; comps.append({'id':c['id'],'treatment':t,'control':ctl,'exact_rate_delta':agg[t]['primary_exact_rate']-agg[ctl]['primary_exact_rate'],'field_accuracy_delta':agg[t]['primary_field_accuracy']-agg[ctl]['primary_field_accuracy'],'stale_error_delta':agg[t]['stale_value_error_error_count']-agg[ctl]['stale_value_error_count'] if 'stale_value_error_error_count' in agg[t] else agg[t]['stale_value_error_count']-agg[ctl]['stale_value_error_count'],'omission_delta':agg[t]['omission_count']-agg[ctl]['omission_count'],'invention_delta':agg[t]['invention_count']-agg[ctl]['invention_count']})
 return agg,comps
def main():
 ap=argparse.ArgumentParser(); ap.add_argument('--execution-config',required=True); ap.add_argument('--problem-pack',required=True); ap.add_argument('--model-profile',required=True); ap.add_argument('--retrieval-mode',required=True); ap.add_argument('--output',default='benchmark_result.json'); ap.add_argument('--validate-only',action='store_true'); a=ap.parse_args()
 cfg,manifest,registry,problems,_root,tmp=contract.load_contract(Path(a.execution_config),Path(a.problem_pack),a.model_profile,a.retrieval_mode)
 try:
  contract.validate_contract(cfg,problems); selected=next(x for x in registry['profiles'] if x['id']==a.model_profile)
  if a.validate_only:
   print(json.dumps({'status':'VALIDATION_PASS','suite_id':manifest.get('suite_id'),'project_count':len(problems),'model_profile_id':a.model_profile,'retrieval_mode':a.retrieval_mode,'routing_conditions':[x['id'] for x in cfg['routing_conditions']],'bank_counts_observed':sorted(set(len(p['bank_profile']['banks']) for p in problems)),'families_observed':sorted(set(str(p.get('family')) for p in problems))},indent=2,sort_keys=True)); return
  import torch
  ...
  os.environ['RWKV_V7_ON']='1'; os.environ['RWKV_JIT_ON']='1'; os.environ['RWKV_CUDA_ON']='0'; strategy='cuda fp16'; model=RWKV(model=str(path)[:-4],strategy=strategy); pipe=PIPELINE(model,'rwkv_vocab_v20230424'); results=[]
  for p in problems:
   ...
   results.append(row)
  agg,comps=aggregate_results(cfg,results); out={'schema':cfg['result_schema'],'status':'COMPLETE','scientific_evidence':True,'source_commit':os.environ.get('VELA_SOURCE_COMMIT'),'suite_id':manifest.get('suite_id'),'model_profile':selected,'retrieval_mode':a.retrieval_mode,'runtime':{'torch':torch.__version__,'cuda':torch.version.cuda,'device':torch.cuda.get_device_name(0),'device_capability':list(torch.cuda.get_device_capability(0)),'strategy':strategy},'project_count':len(results),'projects':results,'aggregate':agg,'comparisons':comps}; Path(a.output).write_text(json.dumps(out,indent=2,sort_keys=True)+'\n'); print(json.dumps({'status':'COMPLETE','aggregate':agg,'comparisons':comps},indent=2,sort_keys=True))
 finally:
  if tmp: tmp.cleanup()
