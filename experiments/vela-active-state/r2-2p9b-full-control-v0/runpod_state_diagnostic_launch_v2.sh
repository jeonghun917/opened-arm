...
if [[ "${VELA_RUNPOD_DIAGNOSTIC_AUTHORIZED:-false}" != "true" ]]; then
  echo "RUNPOD_DIAGNOSTIC_BLOCKED: canonical diagnostic authority not present" >&2
  exit 77
fi
...
ENV_JSON="$(python - "$BOOTSTRAP_B64" "$VELA_SOURCE_COMMIT" "$DIAG_URL" <<'PY'
import json,sys
b64,source,url=sys.argv[1:]
print(json.dumps({'VELA_BOOTSTRAP_B64':b64,'VELA_SOURCE_COMMIT':source,'VELA_DIAG_URL':url,'PYTHONUNBUFFERED':'1'},separators=(',',':')))
PY
)"
DOCKER_ARGS="sh -lc 'echo \"\$VELA_BOOTSTRAP_B64\" | base64 -d | timeout ${WORKLOAD_TIMEOUT_SECONDS}s sh'"

name="vela-p-r2-02-state-diag-${VELA_SOURCE_COMMIT:0:12}"
...
while (( $(date +%s) < DEADLINE )); do
  ...
  if python - "$EVIDENCE_DIR/pod-logs.jsonl" "$EVIDENCE_DIR/diagnostic-result.json" <<'PY'
import json,sys
src,out=sys.argv[1:]
result=None
...
if [[ "$FOUND" != "1" ]]; then
  echo "RUNPOD_DIAGNOSTIC_RESULT_TIMEOUT" >&2
  exit 78
fi

python - "$EVIDENCE_DIR/diagnostic-result.json" "$VELA_SOURCE_COMMIT" <<'PY'
import json,sys
r=json.load(open(sys.argv[1])); source=sys.argv[2]
assert r.get('status')=='DIAGNOSTIC_COMPLETE', r
assert r.get('scientific_evidence') is False, r
assert r.get('source_commit')==source, r
assert 'A40' in ((r.get('runtime') or {}).get('device') or ''), r
assert (r.get('runtime') or {}).get('torch')=='2.7.1+cu126', r
m=r.get('model') or {}
assert m.get('size')==5896274949, m
assert m.get('sha256')=='df5716263b617e7da83590446fb2a98b6663447cfd52e8e9b49e11ce7f4faa3e', m
...
env={
 'status':'DIAGNOSTIC_COMPLETE','scientific_evidence':False,'provider':'RUNPOD','phase':'STATE_DIAGNOSTIC_ONLY',
 ...
 'diagnostic':json.load(open(p/'diagnostic-result.json')),
 'image_ref':(p/'image-ref.txt').read_text().strip()
}
json.dump(env,open(p/'diagnostic-envelope.json','w'),indent=2,sort_keys=True)
