...
python - "$EVIDENCE_DIR/gpu-list.json" "$EVIDENCE_DIR/selected-gpu.json" "$GPU_ID" "$GPU_PRICE_CAP" "$VELA_RUNPOD_DATA_CENTER_ID" <<'PY'
import json,sys
src,out,gpu_id,cap,dc_id=sys.argv[1:]
gpus=json.load(open(src)); cap=float(cap)
...
if dc.get('stockStatus') not in {'High','Medium'}:
    raise SystemExit(f'DATACENTER_STOCK_NOT_SUFFICIENT: {dc_id}={dc.get("stockStatus")}')
json.dump({
    'gpuId':match.get('gpuId'),
    'displayName':match.get('displayName'),
    'memoryInGb':match.get('memoryInGb'),
    'secureCloud':match.get('secureCloud'),
    'securePricePerHr':match.get('securePricePerHr'),
    'stockStatus':match.get('stockStatus'),
...
for i,x in enumerate(chunks,1):
    print(f'VELA_CONDITION_RESULT_CHUNK={i:04d}/{len(chunks):04d}:'+x,flush=True)
print('VELA_SCIENCE_RESULT_READY='+json.dumps({
    'condition':c,
    'sha256':meta['sha256'],
    'chunks':meta['chunks'],
    'scientific_evidence':bool(json.loads(raw).get('scientific_evidence')) if raw else False,
...
print(json.dumps({
    'VELA_BOOTSTRAP_B64':b64,
    'VELA_SOURCE_COMMIT':source,
    'VELA_BASE_RAW':base,
    'VELA_CONDITION':condition,
    'VELA_POST_RESULT_HOLD_SECONDS':hold,
    'VELA_MAX_CHUNK_CHARS':chunk,
    'PYTHONUNBUFFERED':'1'
},separators=(',',':')))
PY
)"
...
print(
  '1' if x.get('ready_seen') else '0',
  '1' if x.get('bootstrap_exit_seen') else '0',
  x.get('bootstrap_exit') if x.get('bootstrap_exit') is not None else 'NA',
  '1' if x.get('result_reconstructed') else '0',
  '1' if x.get('validation_errors') else '0',
)
PY
)
EOF
  ...
  if [[ "$ready_seen" == "1" || "$exit_seen" == "1" ]]; then
    ...
    if [[ -s "$EVIDENCE_DIR/result-ready-final-logs.jsonl" ]]; then
      cat "$EVIDENCE_DIR/result-ready-final-logs.jsonl" >> "$ACCUM"
      awk '!seen[$0]++' "$ACCUM" > "${ACCUM}.tmp"
      mv "${ACCUM}.tmp" "$ACCUM"
    fi
...
EOF
    ...
    if [[ "$validation_error" == "1" ]]; then
      TERMINAL_REASON="TRANSPORT_VALIDATION_ERROR"
      break
    fi
    if [[ "$exit_code" != "NA" && "$exit_code" != "0" ]]; then
      TERMINAL_REASON="BOOTSTRAP_EXIT_${exit_code}"
      break
    fi
    if [[ "$reconstructed" == "1" ]]; then
      FOUND=1
      TERMINAL_REASON="RESULT_RECONSTRUCTED_AFTER_PROVIDER_DELETE"
      break
    fi
    TERMINAL_REASON="RESULT_READY_BUT_RECONSTRUCTION_FAILED_AFTER_PROVIDER_DELETE"
    break
  fi
...
from pathlib import Path
p=Path(sys.argv[1])
c,source=sys.argv[2],sys.argv[3]
start,end=int(sys.argv[4]),int(sys.argv[5])
price=float(sys.argv[6]); max_runtime=int(sys.argv[7]); terminal=sys.argv[8]
rp=p/f'condition_result_{c}.json'
raw=rp.read_bytes(); r=json.loads(raw); elapsed=max(0,end-start)
env={
  'status':'PASS' if r.get('status')=='P-R2-02_CONDITION_RESULT' and r.get('scientific_evidence') is True else 'FAIL',
  'scientific_evidence':bool(r.get('scientific_evidence')),
  'provider':'RUNPOD',
  'phase':'SCIENTIFIC_CONDITION',
  'condition':c,
  ...
  'condition_result_sha256':hashlib.sha256(raw).hexdigest(),
  'transport_terminal_reason':terminal,
}
