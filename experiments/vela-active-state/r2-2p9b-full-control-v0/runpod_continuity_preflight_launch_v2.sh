...
if [[ "${VELA_RUNPOD_CONTINUITY_V2_AUTHORIZED:-false}" != "true" ]]; then
  echo "RUNPOD_CONTINUITY_V2_BLOCKED: canonical authority not present" >&2
  exit 77
fi
...
EVIDENCE_DIR="${VELA_CONTINUITY_V2_EVIDENCE_DIR:-/tmp/runpod-continuity-v2-evidence}"
...
urllib.request.urlretrieve(os.environ['VELA_CONTINUITY_V2_URL'], 'runpod_continuity_preflight_v2.py')
PY
rc=0
...
echo "VELA_CONTINUITY_V2_BOOTSTRAP_EXIT=$rc"
...
print(json.dumps({'VELA_BOOTSTRAP_B64':b64,'VELA_SOURCE_COMMIT':source,'VELA_CONTINUITY_V2_URL':url,'VELA_POST_RESULT_HOLD_SECONDS':hold,'PYTHONUNBUFFERED':'1'},separators=(',',':')))
PY
)"
DOCKER_ARGS="sh -lc 'echo \"\$VELA_BOOTSTRAP_B64\" | base64 -d | timeout ${WORKLOAD_TIMEOUT_SECONDS}s sh'"

name="vela-p-r2-02-cont-v2-${VELA_SOURCE_COMMIT:0:12}"
...
while (( $(date +%s) < DEADLINE )); do
  ...
  if python - "$EVIDENCE_DIR/pod-logs.jsonl" "$EVIDENCE_DIR/continuity-v2-result.json" <<'PY'
import json,sys
...
if [[ "$FOUND" != "1" ]]; then
  echo "RUNPOD_CONTINUITY_V2_RESULT_TIMEOUT" >&2
  exit 78
fi
...
elapsed=max(0,end-start); result=json.load(open(p/'continuity-v2-result.json'))
env={
 'status':result.get('status'),'classification':result.get('classification'),
 'scientific_evidence':False,'provider':'RUNPOD','phase':'CONTINUITY_PREFLIGHT_V2_ONLY',
...
json.dump(env,open(p/'continuity-v2-envelope.json','w'),indent=2,sort_keys=True)
print(json.dumps(env,separators=(',',':'),sort_keys=True))
if result.get('status')!='PASS': raise SystemExit(80)
PY
