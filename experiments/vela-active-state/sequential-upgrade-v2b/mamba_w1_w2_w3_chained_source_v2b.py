...
def choose_w3_only_targets(w2_eval, w3_eval):
    w2 = row_map(w2_eval)
    w3 = row_map(w3_eval)
    candidates = []
...
def history_and_probe(target):
    ...
    if not hist.endswith(" "):
        hist += " "
    return hist, ("w3_only_correction", "\n" + query, list(target["candidates"]), target["expected"])
...
def run():
    ...
    try:
        ...
        targets = choose_w3_only_targets(w2_eval, w3_eval)
        if not targets:
            write_report({"status":"VELA_SEQUENTIAL_CHAINED_SOURCE_V2B_NO_W3_ONLY_TARGET","model":v3.MODEL_ID,"suite_pass":False})
            raise SystemExit(2)
        ...
        hop1 = v1.target_free_select(model, tok, prefix_ids, prefix_starts, prefix_ends, v1.clone_cache_map(w1_prefix_lineage), prefix_specs, prefix_segments)
        ...
        w2_prefix_lineage, w2_prefix_origins, w2_prefix_current = v1.rebuild_lineage(model, prefix_ids, prefix_starts, prefix_ends, v1.clone_cache_map(w1_prefix_lineage), dict(w1_prefix_origins), hop1["chosen_seg"], "W2")
        ...
        for idx, variant in enumerate(VARIANTS):
            ...
            v1.load_xproj(model, w2_weights)
            w2_lineage, w2_origins, w2_carried_current = extend_w2_lineage(model, ids, starts, ends, w2_prefix_lineage, w2_prefix_origins, first_suffix_seg, w2_prefix_current)
            with torch.no_grad():
            ...
            _, w3_origins, w3_current = v1.rebuild_lineage(model, ids, starts, ends, w2_lineage, w2_origins, hop2["chosen_seg"], "W3")
            ...
            fixture_pass = bool(hop1_comp["decision_agreement"] == 1.0 and hop1_state_error["rms"] > EPS and carried_error["rms"] > EPS and native_acc == 1.0 and hop2_origin == "W2" and hop2_comp["decision_agreement"] == 1.0 and chain_comp["decision_agreement"] == 1.0)
            rows.append({
                "fixture": variant,
                "history_tokens": T,
                "w2_era_start_pos": prefix_T,
                "w3_only_target": {"pair":target["pair"],"variant":target["variant"],"expected":target["expected"],"w2_heldout_correct":row_map(w2_eval)[(target["pair"],target["variant"])]["correct"],"w3_heldout_correct":row_map(w3_eval)[(target["pair"],target["variant"])]["correct"]},
                "hop1_w1_to_w2_prefix": {"selected_anchor_event":hop1["chosen_seg"],"anchor_pos":hop1["anchor_pos"],"vs_w2_native":hop1_comp,"state_error_vs_w2_native_prefix":hop1_state_error},
                "w2_carried_before_w3": {"state_error_vs_w2_native_full":carried_error,"origin_at_w2_era_start":w2_origins[prefix_T],"origin_at_final_before_w3":w2_origins[T]},
                "hop2_actual_chained_lineage_to_w3": {"selected_anchor_event":hop2["chosen_seg"],"anchor_pos":hop2["anchor_pos"],"anchor_source_generation":hop2_origin,"anchor_is_in_w2_era":bool(hop2["anchor_pos"]>=prefix_T),"selected_segments":hop2["selected_segments"],"prune_trace":hop2["prune_trace"],"vs_w3_native":hop2_comp},
                "w3_native_probe_rows": w3_native_rows,
                "w3_native_expected_accuracy": native_acc,
                "final_chain_w3": {"functional_vs_w3_native":chain_comp,"state_error_vs_w3_native":v3.cache_distance(w3_current,w3_native),"lineage_origin_at_final":w3_origins[T]},
                "direct_w1_to_w3_control": {"selected_anchor_event":direct["chosen_seg"],"anchor_pos":direct["anchor_pos"],"vs_w3_native":direct_comp,"chain_vs_direct_decision_agreement":v1.agreement(chain_rows,direct["rows"])},
        ...
        suite_pass = bool(strict_gain and len(rows)>=3 and w2_source_count==len(rows) and valid_native_count==len(rows) and passed_count==len(rows))
        report = {
            "status":"VELA_SEQUENTIAL_W1_W2_W3_CHAINED_SOURCE_V2B",
            "model":v3.MODEL_ID,"torch_version":torch.__version__,"transformers_version":transformers_version,
            "generations":{"W1":{"training_epoch":0,"accuracy":w1_eval["accuracy"],"correction_accuracy":w1_eval["correction_accuracy"],"control_accuracy":w1_eval["control_accuracy"]},"W2":{"training_epoch":v1.W2_EPOCH,"accuracy":w2_eval["accuracy"],"correction_accuracy":w2_eval["correction_accuracy"],"control_accuracy":w2_eval["control_accuracy"]},"W3":{"training_epoch":v1.W3_EPOCH,"accuracy":w3_eval["accuracy"],"correction_accuracy":w3_eval["correction_accuracy"],"control_accuracy":w3_eval["control_accuracy"]},"epoch_mean_loss":losses},
            "gate":{"strict_two_step_capability_gain":strict_gain,"fixture_count":len(rows),"distinct_w3_only_target_count":len(targets),"w2_origin_hop2_anchor_count":w2_source_count,"w2_origin_hop2_anchor_rate":w2_source_count/max(len(rows),1),"w3_native_valid_fixture_count":valid_native_count,"fixture_pass_count":passed_count,"fixture_pass_rate":passed_count/max(len(rows),1),"suite_pass":suite_pass},
            "fixtures":rows,
            "success_definition":"At least three fixtures must carry a nonzero W1->W2 migration-state difference into the W2 era. Each fixture uses only a held-out correction probe that is wrong in W2 and correct in W3, selected before any anchor scoring. The unchanged W3 target-free selector must naturally select a source_generation=W2 checkpoint on every fixture and replay from that W2-origin state to 100% W3-native decision agreement. Direct W1->W3 is control only.",
            "claim_boundary":"Three synthetic Mamba-130M chained-source histories that isolate W3-only correction behavior. The failed v2 auxiliary project/action probes are not part of this gate. This tests true W2-origin chained-state accumulation for the current migration policy; it does not establish generic long-horizon robustness, final-backbone fitness, or identity continuity."
        }
        write_report(report)
        if not suite_pass:
            raise SystemExit(3)
        ...
        write_report({"status":"VELA_SEQUENTIAL_W1_W2_W3_CHAINED_SOURCE_V2B_ERROR","model":getattr(v3,"MODEL_ID",None),"torch_version":torch.__version__,"transformers_version":transformers_version,"error_type":type(exc).__name__,"error":str(exc),"traceback_tail":traceback.format_exc().splitlines()[-45:]})
        raise
