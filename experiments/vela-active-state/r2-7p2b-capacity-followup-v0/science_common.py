...
BASE_SCIENCE_COMMON_PATH = Path(__file__).resolve().parent.parent / "r2-2p9b-full-control-v0" / "science_common.py"

def _gitblob(raw: bytes) -> str:
    return hashlib.sha1(b"blob " + str(len(raw)).encode() + b"\0" + raw).hexdigest()

_raw = BASE_SCIENCE_COMMON_PATH.read_bytes()
_actual = _gitblob(_raw)
if _actual != BASE_SCIENCE_COMMON_BLOB:
    raise RuntimeError(f"frozen P-R2-02 science_common drift: {_actual}")

_spec = importlib.util.spec_from_file_location("_vela_pr202_science_common", BASE_SCIENCE_COMMON_PATH)
