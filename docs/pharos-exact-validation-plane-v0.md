...
1. resolves the exact requested SHA;
2. requires a Pharos-specific private checkout credential;
3. checks out only `jeonghun917/pharos-orbis` at that SHA with persisted credentials disabled;
...
6. runs `bash ops/validation/recipe-v0.sh`;
7. requires exit code 0 and a clean working tree afterward;
8. publishes the final commit status and writes bounded evidence to `ops/pharos-exact-validation.json`.
...
The required secret is `PHAROS_REPO_TOKEN`. It must be explicitly bound for private Pharos checkout/status publication. Dashboard credentials must not be reused by assumption. Checkout uses `persist-credentials: false`; the recipe step receives no credential environment variable from this workflow.
