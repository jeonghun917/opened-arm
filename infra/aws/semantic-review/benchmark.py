...
def prompt_for(fixture: dict) -> str:
    return f"""You are a code-review classifier. Review only the supplied requirements and snippet.
...
Rules:
- Treat the stated requirements as authoritative context.
...
- Do not invent requirements or hidden behavior beyond what is stated.
...
def summarize(model: dict, rows: list[dict], fixtures: list[dict]) -> dict:
    ...
    exact = sum(
        r["schema_valid"]
        and r["verdict"] == fixture_by_id[r["fixture_id"]]["expected_verdict"]
    ...
    false_pos = sum(r["schema_valid"] and r["verdict"] == "BUG" for r in clean)
    schema_valid = sum(r.get("schema_valid", False) for r in completed)
    ...
    schema_rate = schema_valid / len(completed) if completed else 0.0
    ...
    quality = 0.65 * recall + 0.15 * (1.0 - fpr) + 0.10 * consistency + 0.10 * schema_rate

    expected_count = len(fixtures) * REPEATS
    eligible = (
...
def main() -> int:
        ...
        for fixture in fixtures:
            for repeat in range(1, REPEATS + 1):
                ...
                try:
                    ...
                    row = {**base, **result}
                    print(json.dumps({k: row[k] for k in ("model", "fixture_id", "repeat", "verdict", "category", "schema_valid", "input_tokens", "output_tokens", "latency_ms")}))
                except Exception as exc:
                    message = str(exc)
    ...
    output = {
        "benchmark": "semantic-review-benchmark-v0.1",
